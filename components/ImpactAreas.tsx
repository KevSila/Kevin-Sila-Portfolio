      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 border-b border-border/20 pb-8">
          <div className="space-y-3">
            ...
            <h3 className="text-textLight text-3xl md:text-4xl font-semibold tracking-tight">Impact Areas</h3>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-4 px-6 py-3.5 rounded-[2px] text-left transition-all duration-300 border ${
                      isActive
                        ? 'bg-accent text-primary border-accent shadow-[0_4px_24px_rgba(0,0,0,0.05)]'
                        : 'bg-secondary text-textDim border-border/40 hover:text-textLight hover:border-border'
                    }`}
                  >
                    <div className={isActive ? 'text-primary' : 'text-textLight opacity-80'}>
                      {getIcon(area.id, 14)}
                    </div>
          ...
          <div className="lg:col-span-8">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="bg-secondary border border-border/45 p-8 rounded-[2px] space-y-3">
                  <h4 className="text-textMuted text-[10px] font-semibold uppercase tracking-widest">Capability Scope</h4>
                  <p className="text-textLight text-lg md:text-xl font-light tracking-normal leading-relaxed">
                ...
                <div className="space-y-6">
                  {activeArea.cards.map((card, i) => (
                    <motion.div
                      ...
                      className="p-8 border border-border/45 bg-secondary rounded-[2px] transition-all hover:bg-accentSoft hover:border-border/80 group"
                    >
                      <div className="space-y-4">
                        {/* Header Details */}
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                          <div className="space-y-1.5">
                            <h5 className="text-textLight text-lg md:text-xl font-medium tracking-tight">{card.title}</h5>
                            {(card.role || card.organizations) && (
                              <p className="text-textDim text-xs font-light">
                                {card.role && <span className="text-textLight font-medium opacity-90">{card.role}</span>}
                                {card.organizations && <span className="opacity-75"> — {card.organizations}</span>}
                              </p>
                            )}
                          </div>
                          {card.year && (
                            <span className="text-textMuted text-[9px] font-mono whitespace-nowrap bg-accentSoft border border-border/40 px-2.5 py-1 rounded-[2px]">
                              {card.year}
                            </span>
                          )}
                        </div>
                        ...
                        {/* Details Description */}
                        {card.details && (
                          <p className="text-textDim text-sm leading-relaxed font-light opacity-95 pt-4 border-t border-border/30">
                            {card.details}
                          </p>
                        )}
                        ...
                        {activeArea.id === 'strategy-docs' && i === 0 && (
                          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 pt-4 border-t border-border/30">
                            {[
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                          {card.tags.map((tag) => (
                            <span
                              key={tag}
                              className="text-[9px] font-mono tracking-wider text-textMuted border border-border/40 px-2 py-0.5 bg-primary rounded-[2px]"
                            >
                              {tag}
                            </span>
                          ))}
